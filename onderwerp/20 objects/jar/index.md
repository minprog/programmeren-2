...
*   `__str__` should return a `str` with $$n$$ cookie emoji, where $$n$$ is the number of cookies in the cookie jar. For instance, if there are 3 cookies in the cookie jar, then `str` should return the following string:
...
Note that it's not as easy to test instance methods as it is to test functions alone, since instance methods sometimes manipulate the same "state" (i.e., instance variables). To test one method (e.g., `withdraw`), then, you might need to call another method first (e.g., `deposit`). But the method you call first might itself not be correct! Don't worry about that for now.

Here are some sensible tests to implement:
