...
1. Execute `mkdir module10` to make (i.e., create) a directory called `module10` in your home directory, if you haven't already done so.
1. Execute `cd module10` to change into (i.e., open) that directory.
...
- `cd module10`
- `zip -r survey.zip survey`

Now your new file `survey.zip` will be put next to the `survey` directory. You can download it via the Download option in the CS50 IDE. Make sure that the file is not unzipped on your own computer, but instead left as-is, so you can add it to the form below.
